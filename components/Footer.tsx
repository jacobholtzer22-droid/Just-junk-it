    ...
    <footer className="border-t-2 border-paper/10 bg-surface px-5 py-14 sm:px-6">
      <div className="mx-auto grid max-w-6xl gap-10 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="mt-4 font-display text-lg uppercase text-accent">{business.tagline}</p>
        </div>
        ...
        <div>
          ...
          <ul className="mt-2">
            ...
            <li>
              <SmsLink className="-mx-2 flex min-h-[44px] items-center gap-2 px-2 text-lg text-paper hover:text-accent">
                <MessageSquare className="h-4 w-4 shrink-0" aria-hidden="true" />
                Text us
              </SmsLink>
            </li>
            <li>
              <a href={`mailto:${business.email}`} className="-mx-2 flex min-h-[44px] items-center gap-2 break-all px-2 text-paper/75 hover:text-accent">
        ...
        <div>
          ...
          <ul className="mt-2">
            {services.map((s) => (
              <li key={s.slug}>
                <Link href={`/${s.slug}`} className="-mx-2 flex min-h-[44px] items-center px-2 text-paper/75 hover:text-accent">
                  {s.title}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-2">
            {nav.map((n) => (
              <li key={n.href}>
                <Link href={n.href} className="-mx-2 flex min-h-[44px] items-center px-2 text-paper/75 hover:text-accent">
                  {n.label}
                </Link>
              </li>
            ))}
            <li>
              <Link href="/privacy" className="-mx-2 flex min-h-[44px] items-center px-2 text-paper/75 hover:text-accent">
                Privacy
              </Link>
            </li>
          </ul>
          ...
          <ul className="mt-3 flex gap-5">
            {business.social.facebook ? (
              <li>
                <a href={business.social.facebook} className="-mx-2 flex min-h-[44px] items-center px-2 text-paper/75 hover:text-accent">Facebook</a>
              </li>
            ) : null}
            {business.social.instagram ? (
              <li>
                <a href={business.social.instagram} className="-mx-2 flex min-h-[44px] items-center px-2 text-paper/75 hover:text-accent">Instagram</a>
              </li>
            ) : null}
          </ul>
        </div>
      ...
      <div className="mx-auto mt-12 max-w-6xl border-t-2 border-paper/10 pt-6 text-sm text-paper/60">
        ...
        <p className="mt-2">
          <a href={footer.builtByHref} className="inline-flex min-h-[44px] items-center hover:text-accent">{footer.builtBy}</a>
        </p>
      </div>
    </footer>
  );
